...
class RAGFlowPdfParser:
    def __init__(self):
        ...
        # 尝试加载XGBoost模型（用于文本连接优化，可选）
        try:
            model_dir = os.path.join("deepdoc_model")
            model_path = os.path.join(model_dir, "updown_concat_xgb.model")
            ...
            if os.path.exists(model_path):
                ...
                print(f"⚠️  XGBoost模型文件不存在，跳过文本连接优化")
                print(f"📁 这不会影响基本的PDF解析功能")
                # 创建一个虚拟的模型对象，避免后续代码出错
                self.updown_cnt_mdl = None
        except Exception as e:
            print(f"⚠️  加载XGBoost模型失败: {e}")
            print(f"📄 继续使用基础PDF解析功能")
            self.updown_cnt_mdl = None
                ...
                while i < min(dp + 12, len(boxes)):
                    ...
                    if self.updown_cnt_mdl is None:
                        ...
                        if vertical_gap > avg_height * 1.5:  # 垂直间距大于平均高度的1.5倍
                            i += 1
                            continue
                    elif self.updown_cnt_mdl.predict(
