...
class OCR(object):
    def __init__(self, model_dir=None):
        ...
        """
        print(f"ðŸ”§ OCRåˆå§‹åŒ–å¼€å§‹...")
        
        if not model_dir:
            # é¦–å…ˆå°è¯•ä½¿ç”¨æœ¬åœ°æ¨¡åž‹ - ä½¿ç”¨ç»å¯¹è·¯å¾„
            script_dir = os.path.dirname(os.path.abspath(__file__))
            project_root = os.path.dirname(os.path.dirname(script_dir))
            local_model_dir = os.path.join(project_root, "deepdoc_model")
            print(f"ðŸ“ æ£€æŸ¥æœ¬åœ°æ¨¡åž‹ç›®å½•: {local_model_dir}")
            
            # æ£€æŸ¥å¿…è¦çš„æ¨¡åž‹æ–‡ä»¶æ˜¯å¦å­˜åœ¨
            required_models = ['det.onnx', 'rec.onnx']
            local_models_exist = all(os.path.exists(os.path.join(local_model_dir, model)) for model in required_models)
            
            if local_models_exist:
                print(f"âœ… æ‰¾åˆ°æœ¬åœ°æ¨¡åž‹æ–‡ä»¶ï¼Œç›´æŽ¥ä½¿ç”¨")
                try:
                    self.text_detector = TextDetector(local_model_dir)
                    self.text_recognizer = TextRecognizer(local_model_dir)
                    print(f"âœ… æœ¬åœ°æ¨¡åž‹åŠ è½½æˆåŠŸ")
                except Exception as e:
                    print(f"âš ï¸  æœ¬åœ°æ¨¡åž‹åŠ è½½å¤±è´¥: {e}")
                    print(f"ðŸŒ å°è¯•ä¸‹è½½è¿œç¨‹æ¨¡åž‹...")
                    try:
                        model_dir = snapshot_download(repo_id="InfiniFlow/deepdoc",
                                                      local_dir=os.path.join("deepdoc_model"),
                                                      local_dir_use_symlinks=False)
                        self.text_detector = TextDetector(model_dir)
                        self.text_recognizer = TextRecognizer(model_dir)
                        print(f"âœ… è¿œç¨‹æ¨¡åž‹ä¸‹è½½å¹¶åŠ è½½æˆåŠŸ")
                    except Exception as download_error:
                        print(f"âŒ è¿œç¨‹æ¨¡åž‹ä¸‹è½½å¤±è´¥: {download_error}")
                        raise
            else:
                print(f"âš ï¸  æœ¬åœ°æ¨¡åž‹æ–‡ä»¶ä¸å®Œæ•´ï¼Œå°è¯•ä¸‹è½½...")
                missing_models = [model for model in required_models 
                                  if not os.path.exists(os.path.join(local_model_dir, model))]
                print(f"ðŸ“„ ç¼ºå¤±çš„æ¨¡åž‹: {missing_models}")
                try:
                    model_dir = snapshot_download(repo_id="InfiniFlow/deepdoc",
                                                  local_dir=os.path.join("deepdoc_model"),
                                                  local_dir_use_symlinks=False)
