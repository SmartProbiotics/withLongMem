...
class TableStructureRecognizer(Recognizer):
    ...
    def __init__(self):
        try:
            ...
            model_dir = os.path.join(project_root, "deepdoc_model")
            print(f"📁 TableStructureRecognizer使用模型目录: {model_dir}")
            super().__init__(self.labels, "tsr", model_dir)
        except Exception as e:
            print(f"⚠️  TableStructureRecognizer模型加载失败: {e}")
            print(f"📄 尝试使用相对路径...")
            try:
                super().__init__(self.labels, "tsr", os.path.join("deepdoc_model"))
            except Exception as e2:
                print(f"❌ TableStructureRecognizer模型加载失败: {e2}")
                exit(1)
